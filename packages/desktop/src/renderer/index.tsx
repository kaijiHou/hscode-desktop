...
// HSCode: Sentry 崩溃上报已禁用 —— 隐私清理，不向任何外部服务上报错误

const [updaterState, setUpdaterState] = createSignal<UpdaterState>({ status: "disabled" })
void window.api.updater.subscribe(setUpdaterState)

const deepLinkEvent = "hscode:deep-link"
...
function windowLastActiveUrlKey(windowID: string) {
  return `hscode.desktop.window.${windowID}.last-active-url`
}
...
function LoadingSplash() {
  return (
    <div class="h-dvh w-screen flex flex-col items-center justify-center bg-background-base">
      <HSCodeLoadingMark class="size-16 opacity-50 animate-pulse" />
    </div>
  )
}
...
function DesktopRoot(props: { windowState: DesktopWindowState }) {
  ...
  const loadLocale = async () => {
    const current = await platform.storage?.("hscode.global.dat").getItem("language")
