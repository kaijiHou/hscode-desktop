...
const main = Effect.gen(function* () {
  ...
  // HSCode Phase 1.9: passive models.dev catalog fetch DISABLED by default.
  // Provider metadata is bundled at build time (models-dev-standalone.json).
  // User-triggered provider requests (DeepSeek API, OpenCode Go, self-hosted) are still allowed.
  ...
  app.setName(app.isPackaged ? APP_NAMES[CHANNEL] : "HSCode Dev")
  ...
  app.on("second-instance", (_event: Event, argv: string[]) => {
    const urls = argv.filter((arg: string) => arg.startsWith("hscode://"))
    if (urls.length) {
  ...
  app.setAsDefaultProtocolClient("hscode")
  registerRendererProtocol()
  setDockIcon()
